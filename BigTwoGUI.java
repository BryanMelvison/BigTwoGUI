...
public class BigTwoGUI {
	 ...
	 * and (iii) enable the BigTwoPanel for selection of cards through mouse clicks.
	 */
	 ...
	 * (i.e., making them not clickable); (ii) disable the chat input; 
	...
	public class BigTwoPanel extends JPanel implements MouseListener{
		...
		public void paintComponent(Graphics g) {
			...
			g.drawImage(anya[1], 30, 175, bigTwoPanel);
			g.drawLine(1500, 275, 0, 275);
			
			g.drawImage(anya[2], 30, 315, bigTwoPanel);
			g.drawLine(1500, 415, 0, 415);
			
			g.drawImage(anya[3], 30, 455, bigTwoPanel);
		...
		public void mouseReleased(MouseEvent e) {
			...
			int lastcardidx = game.getPlayerList().get(game.getCurrentPlayerIdx()).getNumOfCards() - 1;
			if (e.getX() >= 155 + 40 * lastcardidx && e.getX() <= 255+ 40 * lastcardidx) {
				if (selected[lastcardidx] == false && e.getY() >= 25 + 140 * game.getCurrentPlayerIdx() && e.getY() <= 125+ 140 * game.getCurrentPlayerIdx()) {
					selected[lastcardidx] = true;
					flag = true;
				}
				else if (selected[lastcardidx] == true && e.getY() >= 15 + 140 * game.getCurrentPlayerIdx() && e.getY() <= 115+ 140 * game.getCurrentPlayerIdx()) {
			...
			for (int i = 0; i < game.getPlayerList().get(game.getCurrentPlayerIdx()).getNumOfCards(); i++) {
				if (flag == false) {
					//First case: Check the card that is 4/10 visible:
					if(e.getX() >= 155 + i * 40 && e.getX() <= 195 + i * 40) {
						//if pressed card isn't selected already
						if(selected[i] == false && e.getY() >= 25 + game.getCurrentPlayerIdx() * 140 && e.getY() <= 125 + game.getCurrentPlayerIdx() *140) {
							selected[i] = true;
							flag = true;
						}
						// if it is
						else if(selected[i] == true && e.getY() >= 15 + game.getCurrentPlayerIdx() * 140 && e.getY() <= 115 + game.getCurrentPlayerIdx() *140) {
							selected[i] = false;
							flag = true;
						}
					}
					...
					else if(e.getX() >= 195 + i * 40 && e.getX() <= 255 + i* 40 && e.getY() >= 115 + game.getCurrentPlayerIdx() * 140 && e.getY() <= 125 + game.getCurrentPlayerIdx() *140) {
					...
					else if(e.getX() >= 195 + i * 40 && e.getX() <= 255 + i* 40 && e.getY() >= 15 + game.getCurrentPlayerIdx() * 140 && e.getY() <= 25 + game.getCurrentPlayerIdx() *140) {
	...
	public class RestartMenuItemListener implements ActionListener{
		 ...
		 * BigTwoDeck object as an argument.
		 * 
		 * @param e to handle menu-item-click events for the “Restart” menu item
		 */
	...
	public class ChatKeyListener implements KeyListener{
		...
		public void keyPressed(KeyEvent e) {
			if (e.getKeyCode() == KeyEvent.VK_ENTER) {
				chatArea.append(" Player " + game.getCurrentPlayerIdx() + ": " + chatInput.getText() + "\n");
				chatInput.setText("");
			}
		}
