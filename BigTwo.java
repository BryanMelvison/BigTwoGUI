...
public class BigTwo implements CardGame{
	...
	private void printResults(Hand hand) {
		...
		this.ui.printMsg("{"+ hand.getType().toString() + "} " + cards);
	}
	...
	public void checkMove(int playerIdx, int[] cardIdx) {
		...
		if (cardIdx == null) {
			...
			for (int j = 0; j < cardIdx.length; j++) {
				temp.addCard(this.getPlayerList().get(playerIdx).getCardsInHand().getCard(cardIdx[j]));
			}
			Hand play_hand = this.composeHand(this.getPlayerList().get(playerIdx), temp);
	...
	private void print_gameend() {
		...
		for(int i = 0; i < 4; i ++) {
			if (this.getPlayerList().get(i).getCardsInHand().size() != 0) {
				this.ui.printMsg("Player " + i + " has " + this.getPlayerList().get(i).getCardsInHand().size() + " cards in hand. \n");	
			}
			else {
